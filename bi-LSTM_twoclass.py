...
def get_model(embedding_matrix,params={}):
    ...
    input_1 = Input(shape=(max_len,))
    x = Embedding(len(params['vocab'])+1 + 464, embed_size,
                  weights=[embedding_matrix], trainable=False)(input_1)
    x = Dropout(0.5)(x)
    x = Bidirectional(
...
model_embedding = Word2Vec(X_pinyin_true_train, size= 200, window=1, min_count=1, sg=1)

vocab_list = list(model_embedding.wv.vocab.keys())

word_index = {word: index for index, word in enumerate(vocab_list)}

len_embedding = 16668
def get_pinyin_index(sentence):
    global word_index
    sequence = []
    for word in sentence:
        try:
            sequence.append(word_index[word]+ len_embedding)
        except KeyError:
            pass
    return sequence

x_pinyin_train = list(map(get_pinyin_index, X_pinyin_true_train)) 
x_pinyin_dev = list(map(get_pinyin_index, X_pinyin_true_dev))
x_pinyin_test = list(map(get_pinyin_index, X_pinyin_true_test))
x_pinyin_asrtest = list(map(get_pinyin_index, X_pinyin_test))

maxlen = 3
x_pinyin_train = pad_sequences(x_pinyin_train, maxlen = maxlen)
x_pinyin_dev = pad_sequences(x_pinyin_dev, maxlen = maxlen)
x_pinyin_test = pad_sequences(x_pinyin_test, maxlen = maxlen)
x_pinyin_asrtest = pad_sequences(x_pinyin_asrtest, maxlen = maxlen)
...
#x_train, y_train, x_dev, y_dev, x_test, y_test, embedding_matrix = process_feature(X_train, y_train, X_dev, y_dev, X_test, y_test, text_list, params = params)
ipdb.set_trace()

x_true_train, y_train, x_true_dev, y_dev, x_true_test, y_test, x_test, embedding_matrix = process_feature(X_true_train, y_train, X_true_dev, y_dev, X_true_test, y_test, X_test, true_text, params = params)

x_true_train = np.concatenate((x_true_train, x_pinyin_train), axis = 1)
x_true_dev = np.concatenate((x_true_dev, x_pinyin_dev), axis = 1)
x_true_test = np.concatenate((x_true_test, x_pinyin_test), axis = 1)
x_test = np.concatenate((x_test, x_pinyin_asrtest), axis = 1)

embedding_matrix = np.concatenate((embedding_matrix, model_embedding.wv.vectors), axis = 0)
...
model = train_model(x_true_train, y_train,x_true_dev, y_dev, embedding_matrix, params)
