...
def get_model(embedding_matrix,params={}):
    ...
    input_1 = Input(shape=(max_len,))
    x = Embedding(len(params['vocab'])+1 + params['pinyin'], embed_size,
                  weights=[embedding_matrix], trainable=False)(input_1)
    x = Dropout(0.5)(x)
    x = Bidirectional(
...
#Chinese tone
X_pinyin_train, X_pinyin_dev, X_pinyin_test, X_pinyin_true_test = Convert_pinyin(X_train), Convert_pinyin(X_dev), Convert_pinyin(X_test), Convert_pinyin(X_true_test)
model_embedding = Word2Vec(X_pinyin_train, size= 200, window=1, min_count=1, sg=1)

vocab_list = list(model_embedding.wv.vocab.keys())
params["pinyin"] = len(vocab_list)
word_index = {word: index for index, word in enumerate(vocab_list)}

len_embedding = len(embedding_matrix)
...
x_pinyin_train = list(map(get_pinyin_index, X_pinyin_train)) 
x_pinyin_dev = list(map(get_pinyin_index, X_pinyin_dev))
x_pinyin_test = list(map(get_pinyin_index, X_pinyin_test))
x_pinyin_true_test = list(map(get_pinyin_index, X_pinyin_true_test))
...
x_pinyin_true_test = pad_sequences(x_pinyin_true_test, maxlen = maxlen)
...
x_train = np.concatenate((x_train, x_pinyin_train), axis = 1)
x_dev = np.concatenate((x_dev, x_pinyin_dev), axis = 1)
x_test = np.concatenate((x_test, x_pinyin_test), axis = 1)
x_true_test = np.concatenate((x_true_test, x_pinyin_true_test), axis = 1)
